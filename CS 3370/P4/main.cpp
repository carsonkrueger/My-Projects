...
int main(int argc, char* argv[]) {
    ...
    for(int i = 1; i < argc; ++i){ // for each ini file
        ...
        char ch;
        string var = "";
        // READING INI FILE. For each ini file, run the ini settings on each .dat file
        while(iniFile) {
            ch = iniFile.get();
        ...
        if (vt == -1 || width == -1 || pulseDelta == -1 || dropRatio == -1 || belowDropRatio == -1) {
            cout << endl << "Invalid INI file: missing/invalid properties" << endl;
            continue;
        }
        // READING DAT FILES
        for(size_t j = 0; j < datFiles.size(); ++j){ // for each dat file
            ...
            for (int k = 0; k < siz; ++k) {
                if (k == 0 || k == 1 || k == 2 || k == siz-1 || k == siz-2 || k == siz-3) smoothData.push_back(it[0]);
                else smoothData.push_back((it[-3] + 2*it[-2] + 3*it[-1] + 3*it[0] + 3*it[1] + 2*it[2] + it[3]) / 15);
                it ++;
            }

            cout << endl << datFiles[j] << ":" << endl;
            ...
            // detect pulses and peaks
            for (int k=0; k+2 < siz; ++k) {
            ...
            for (size_t k=0; k+1 < pulses.size(); ++k) {
                ...
                if (pulses[k+1]-pulses[k] <= pulseDelta) {
                    // cout << "checking from peak: " << peaks[k]+1 << ", to pulse start: " << pulses[k+1]-1 << endl;
                    // find the # between the peak and next pulse that are less than the peak height * dropRatio
                    for (int l=peaks[k]+1; l < pulses[k+1]-1; ++l)
                        if (data[l] < data[peaks[k]] * dropRatio) ++betweens;
                    // if the number between is greater than belowDropRatio, omit the pulse
                    if (betweens > belowDropRatio) {
                        cout << "Piggyback found, discard: " << pulses[k] << endl; //erase pulse from vec here
            ...
            for (size_t k=0; k<pulses.size(); ++k) {
                int sum = 0, wid = 0;
                int pos = pulses[k];
                while ((pos < pulses[k+1] || k == pulses.size()-1) && wid < width ) {
                    sum += data[pos];
                    ++pos, ++wid;
                }
                cout << "pulse: " << pulses[k] << " (" << sum << ")" << endl;
            }
        }
    }
}